...
- Repo-local pins (`package.json` + **npm** lockfile) win over the inline fallback
  pins; pnpm/yarn repos silently get the fallbacks. Exception: CI's markdownlint and
  actionlint are marketplace actions, so their versions follow the pinned action (the
  markdownlint hook does prefer a repo-local `markdownlint-cli2`)
...
- Hook names in `lefthook/common.yml` keep their `common-` prefix (collision-safety
  ...
  hooks stay inactive in repos without a config). Exception: `common-trailing-whitespace`
  checks every file type where CI only covers prettier's, because leftover conflict
  markers are worth blocking at commit time
- The `CLAUDE.md size` limit is set in `lint.yml` and mirrored in `common.yml`
...
`common-shellcheck`'s file selection must stay a subset of CI's action-shellcheck;
`scripts/test-shellcheck-selection.sh` gates that, goes red when the action's pin moves,
and says what to re-check.
